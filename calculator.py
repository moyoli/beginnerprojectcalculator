...
class Calculator:
    def __init__(self, master):
        ...
        self.screen = Text(master, state='disabled', width=30, height=3, background="blue", foreground="gray")
        ...
        for row in range(1, 5):
            for column in range(4):
                ...
                count += 1
        # arrange the last button '=' at bottom
        buttons[16].grid(row=5, column=0, columnspan=4)
    def createButton(self, val, write=True, width=7):
        # this function creates a button and takes in one argument,the value that should be on the button
        return Button(self.master, text=val, command=lambda: self.click(val, write), width=width)

    def click(self, text, write):
        # this functions handles what happens when a button is clicked
        # 'write' argument if True means the value 'val' should be written on screen,if None,should not be written on screen
        if write == None:
            # only evaluate the code when there is an equation to be evaluated
    ...
    def insert_screen(self, value, newline = False):
        ...
        # record all values inserted in screen
